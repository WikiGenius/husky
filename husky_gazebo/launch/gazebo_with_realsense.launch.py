...
ARGUMENTS = [
    ...
    DeclareLaunchArgument(
        "realsense_mount",
        default_value="top_plate_link",
        description="Parent link the camera is welded to",
    ),
    DeclareLaunchArgument(
        "realsense_xyz",
        default_value="0 0 0.50",
...
def generate_launch_description() -> LaunchDescription:
    ...
    set_model_path = SetEnvironmentVariable(
        ...
        value=[
            ...
            pkg_share("husky_description"),
            "/..",  # meshes for sensor arch
        ],
    )
    ...
    set_plugin_path = SetEnvironmentVariable(
        ...
        value=[
            ...
            ":",
            pkg_share("realsense_gazebo_plugin"),
            "/lib",
        ],
    )
    ...
    robot_desc = {
        "robot_description": Command(
            [
                ...
                " ",
                "realsense_enabled:=",
                LaunchConfiguration("realsense_enabled"),
                " ",
                "realsense_mount:=",
                LaunchConfiguration("realsense_mount"),
                " ",
                'realsense_xyz:="',
                LaunchConfiguration("realsense_xyz"),
                '"',
                " ",
                'realsense_rpy:="',
                LaunchConfiguration("realsense_rpy"),
                '"',
                " ",
                "name:=husky",
                " ",
    ...
    defer_vel = RegisterEventHandler(
        OnProcessExit(target_action=js_broadcaster, on_exit=[husky_vel])
    )
    ...
    ld.add_action(odom_base_transform)
    ld.add_action(rsp)
